<?php
    ...
    if (isset($_GET['id'])){
        ...
        if (isset($_POST['Done'])) {
            ...
            if (empty($profile)) {
                $sql = "UPDATE Activité SET Titre ='$title', description='$content' WHERE id='$id'";
                $res = $conn->prepare($sql);
                $val= $res->execute();
            }else {
                if ($img_type==="image/png"||$img_type==="image/jpg"||$img_type==="image/jpeg") {
                    move_uploaded_file($tmp_dir, __DIR__."/../images/".$profile);
                    $sql = "UPDATE Activité SET Titre ='$title', description='$content', image ='$profile' WHERE id='$id'";
                    $res = $conn->prepare($sql);
                    $val= $res->execute();
                }else {
                    header("Location:MD_Activité.php?id=$id");
        ...
        $req = "SELECT * FROM Activité WHERE id = '$id'";
        $res1 = $conn->query($req);
        if ($res1) {
            $all = $res1->fetchAll();
...
<div class="registerBox form">
        ...
        <form action="./MD_Activité.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data"> 
            <p>Titre</p>
